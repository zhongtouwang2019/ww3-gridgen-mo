...
if combtier:
    ...
    tier1smc = grd.loadNCsmc(writedir+'/atlantic_AS512L3EUK-sub.nc')
    # visualise the grid
    ...
    combbaset1.label = 'AS512L3EUK-comb'
...
if gridmk3:
    ...
    basesmcrm = grd.loadNCsmc(writedir+'/atlantic_AS512L3EUK-comb.nc')
    # visualise the grid
    grd.plotGridsmc(basesmcrm)

    print('')
    print('*** Deselecting tier cells and removing dry cells in specified regions')
    basesmcrm.label = 'AS512L3EUK'
    basesmcrm.unmarkCells(marker='tier', box=[-16.0,45.0,9.4,61.15], osbox=True, thruzero=False)
